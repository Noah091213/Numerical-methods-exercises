...
struct vectorFunction {
    ...
    VecDoub operator () (VecDoub_I q){
        ...
        equations[5] = (1 + v/(w * q[0])) * tan(q[5]) -tan(q[4]);
...
int main() {    
    ...
    for (int i = 0; i< nValues.size(); i++) {
        ...
        vectorFunction F(v, k, w, alpha, d, nValues[i]);
        newt(q, statusBool, F);
        ...
        if (statusBool == true) {
            ...
            std::cout << "Cable length at resting position = " << q[0] << std::endl;
            std::cout << "String tension in cable at resting = " << q[7] << "\n" << std::endl; 
        }
    }
